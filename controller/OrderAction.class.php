...
class OrderAction extends Action {
	...
	public function update() {
		if (isset($_POST['send'])) $this->_model->update() ? $this->_redirect->succ(Tool::getPrevPage(), '订单修改成功！') : $this->_redirect->error('订单修改失败！');
		if (isset($_GET['id'])) {
	...
	public function delete() {
		if (isset($_GET['id'])) $this->_model->delete() ? $this->_redirect->succ(Tool::getPrevPage(), '订单删除成功！') : $this->_redirect->error('订单删除失败！请确认订单是取消状态！');
	}
